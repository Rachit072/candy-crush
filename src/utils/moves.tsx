export const isColumnOfFour = (
    newBoard: string[],
    boardSize: number,
    forColumnOfFour: number
  ) => {
    for (let i: number = 0; i <= forColumnOfFour; i++) {
      ...
      if (columnOfFour.every((square: number) => newBoard[square] === decidedColor && !isBlank)) {
  ...
  export const checkForRowOfFour = (
    ...
    for (let i = 0; i < boardSize * boardSize; i++) {
      ...
      if (invalidMovesForColumnOfFour.includes(i)) 
        continue;
      if (
  ...
  export const checkForColumnOfThree = (
    newBoard: String[],
    boardSize: number,
    forColumnOfThree: number
  ) => {
    for (let i = 0; i <= forColumnOfThree; i++) {
  ...
  export const checkForRowOfThree = (
    ...
    for (let i = 0; i < boardSize * boardSize; i++) {
      ...
      const decidedColor = newBoard[i];
      const isBlank = newBoard[i] === "";
      if (invalidMovesForColumnOfThree.includes(i)) 
        continue;
